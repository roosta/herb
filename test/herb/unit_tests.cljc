...
(deftest convert-vendors
  (let [input ["webkit" :webkit :ms "moz"]
    ...
    (testing "Converting vendors"
      (is (= (#'herb.impl/convert-vendors input) expected)))))
...
(defn test-fn-1
  ...
  (letfn [(fn-binding [] {:some :map})]
    (#'herb.impl/get-name fn-binding "herb.unit-tests" {})))

(defn test-fn-2
  []
  (#'herb.impl/get-name test-fn-1 "herb.unit-tests" {}))

(defn test-fn-3
  []
  (#'herb.impl/get-name #({}) "herb.unit-tests" {}))
...
(deftest with-style!
  ...
  background: red;
    ...
    (is (= (deref (deref #'herb.runtime/injected-styles))
           {"herb_unit-tests_test-fn-4"
            {:data
             {".herb_unit-tests_test-fn-4"
